...
export function Navbar() {
  ...
  useEffect(() => {
    // Throttle scroll handler using requestAnimationFrame to prevent
  ...
  const handleMobileNavClick = useCallback((label: string) => {
    setOpen(false);
    if (label === "Home") {
      window.scrollTo({ top: 0, behavior: "smooth" });
    }
        ...
        >
          ...
          >
            <div className="h-10 w-10 overflow-hidden rounded-full shadow-soft transition-transform duration-500 group-hover:scale-110 group-hover:rotate-6">
              <img src={resolveImage(contact.logo_url || "/logo.jpeg", 128)} alt="Filipino Food Logo" className="h-full w-full object-cover" />
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 overflow-hidden rounded-xl shadow-soft">
                  <img src={resolveImage(contact.logo_url || "/logo.jpeg", 128)} alt="Filipino Food" className="h-full w-full object-cover" />
                </div>
