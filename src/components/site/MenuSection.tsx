...
const MenuItemCard = React.memo(({ item, inCart, onAdd, onUpdateQty, currency }: { 
  ...
  <Link to={`/product/${item.id}`} className="group relative block overflow-hidden rounded-[2rem] bg-white shadow-card transition-all duration-500 hover:shadow-elegant active:scale-[0.98]">
...
export function MenuSection({ onDataLoaded }: { onDataLoaded?: () => void }) {
  ...
  useEffect(() => {
    if (!loading && onDataLoaded) {
      ...
      const t = setTimeout(onDataLoaded, 100);
