...
import { Search, Star, Plus, Flame, Minus } from "lucide-react";
...
const MenuItemCard = React.memo(({ item, inCart, onAdd, onUpdateQty, currency }: { 
  ...
  <Link to={`/product/${item.id}`} className="group relative block overflow-hidden rounded-[2rem] bg-white border border-black/5 shadow-card transition-all duration-500 hover:shadow-elegant active:scale-[0.98]">
    <div className="relative aspect-square overflow-hidden">
      <img
        src={resolveImage(item.image, 640)}
        alt={item.name}
    ...
    <div className="p-3 pb-4">
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        {item.variants && item.variants.length > 0 ? (
          <div className="flex h-8 items-center justify-center rounded-full bg-tropical/10 px-3 text-[10px] font-bold text-tropical transition group-hover:bg-tropical group-hover:text-white">
            Options
          </div>
        ) : inCart ? (
...
export function MenuSection({ onDataLoaded }: { onDataLoaded?: () => void }) {
  ...
  // This significantly reduces re-renders when typing fast in the search box.
  const handleSearchChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setQ(value);
    clearTimeout(debounceTimer.current);
    debounceTimer.current = setTimeout(() => setDebouncedQ(value), 200);
  }, []);
  ...
  // Sync scroll reveals when loading finishes
  useEffect(() => {
    if (!loading) {
  ...
  // Filter items by search query — uses debounced value to avoid
  // expensive filtering on every keystroke
  const filteredMenu = useMemo(() => {
    if (!debouncedQ) return MENU;
  ...
  // Pre-compute cart lookup map for O(1) access per item instead of O(n) .find()
  ...
  return (
    <section id="menu" className="relative bg-atmosphere scroll-mt-24 pb-8">
      {/* Search Header */}
      <div className="mx-auto max-w-7xl px-4 pt-8 pb-12 text-center">
      ...
      {/* Filter Bar (Static) */}
      <div className="relative z-10 border-b border-white/10 py-3">
      ...
      {/* Menu Content */}
      <div className="mx-auto mt-8 max-w-7xl px-4 min-h-[100vh]">
          ...
          <div className="space-y-16">
            ...
            {filteredMenu.length === 0 && (
              ...
              >
                ...
                <button onClick={() => { setQ(""); setDebouncedQ(""); }} className="mt-6 rounded-full bg-white px-6 py-2 shadow-soft hover:text-tropical transition">
                  Clear Search
                </button>
              </motion.div>
            )}
