...
export type MenuItem = {
  ...
  variants?: { id: string; name: string; price: number }[]; // For product variants like 4 Sticks, 7 Sticks
};

export type CartLine = Omit<MenuItem, 'id'> & { 
  id: string; // Unique cart item ID (original id + variant id if applicable)
...
type CartCtx = {
  items: CartLine[];
  add: (m: MenuItem, variant?: { id: string; name: string; price: number }) => void;
...
const CART_STORAGE_KEY = "filipino_food_cart";
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const add = useCallback((m: MenuItem, variant?: { id: string; name: string; price: number }) =>
    setItems((prev) => {
      const cartItemId = variant ? `${m.id}-${variant.id}` : m.id;
      const found = prev.find((p) => p.id === cartItemId);
      if (found) return prev.map((p) => (p.id === cartItemId ? { ...p, qty: p.qty + 1 } : p));
      
      const { id, ...restOfItem } = m;
      return [...prev, { 
        ...restOfItem, 
        id: cartItemId,
        productId: m.id, 
        qty: 1, 
        price: variant ? variant.price : m.price,
...
export function buildWhatsAppLink(items: CartLine[], subtotal: number, details: CheckoutDetails, whatsappNumber: string) {
  ...
  const lines = items.map((i) => `${i.qty}x ${i.name}${i.variantName ? ` (${i.variantName})` : ''} - ${CURRENCY}${formatPrice(i.price * i.qty)}`).join("%0A");
