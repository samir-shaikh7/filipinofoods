...
import { Minus, Plus, Trash2, ArrowLeft, MessageCircle, MapPin, Loader2, Banknote, Wallet, Building2, ShoppingBag } from "lucide-react";
...
export default function CartPage() {
  ...
  // Use useCallback for form field updaters to reduce inline closure creation
  const updateField = useCallback((field: keyof CheckoutDetails, value: string) => {
  ...
  const submitOrder = useCallback((e?: React.FormEvent) => {
  ...
  }, [form, contact, items, total, clear, navigate]);
                ...
                {items.map((item) => (
                  <div key={item.id} className="flex items-center gap-4 rounded-[2.5rem] bg-white p-4 shadow-card transition hover:shadow-elegant">
                    <div className="h-24 w-24 shrink-0 overflow-hidden rounded-[1.5rem] shadow-sm">
                      <img src={resolveImage(item.image, 256)} alt={item.name} className="h-full w-full object-cover" />
                    </div>
                ...
                <div className="rounded-[2.5rem] bg-white p-8 shadow-card">
                  ...
                  <div className="grid gap-6 sm:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <input required value={form.name} onChange={(e) => updateField("name", e.target.value)} className="w-full rounded-2xl border-none bg-muted/40 px-6 py-4 text-sm font-bold outline-none ring-2 ring-transparent transition focus:bg-white focus:ring-tropical" placeholder="Enter your name" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-bold uppercase tracking-widest text-foreground/40 px-1">Phone Number</label>
                      <input required type="tel" value={form.phone} onChange={(e) => updateField("phone", e.target.value)} className="w-full rounded-2xl border-none bg-muted/40 px-6 py-4 text-sm font-bold outline-none ring-2 ring-transparent transition focus:bg-white focus:ring-tropical" placeholder="e.g. +965 6XXX XXXX" />
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-[2.5rem] bg-white p-8 shadow-card">
                  ...
                  <div className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <textarea required value={form.address} onChange={(e) => updateField("address", e.target.value)} rows={3} className="w-full resize-none rounded-2xl border-none bg-muted/40 px-6 py-4 text-sm font-bold outline-none ring-2 ring-transparent transition focus:bg-white focus:ring-tropical" placeholder="Street name, Building, Floor/Unit No." />
                    </div>
                    <div className="grid gap-6 sm:grid-cols-2">
                      <div className="space-y-2">
                        <label className="text-xs font-bold uppercase tracking-widest text-foreground/40 px-1">Landmark (Optional)</label>
                        <input value={form.landmark} onChange={(e) => updateField("landmark", e.target.value)} className="w-full rounded-2xl border-none bg-muted/40 px-6 py-4 text-sm font-bold outline-none ring-2 ring-transparent transition focus:bg-white focus:ring-tropical" placeholder="e.g. Near Blue Gate" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-xs font-bold uppercase tracking-widest text-foreground/40 px-1">Order Notes</label>
                        <input value={form.notes} onChange={(e) => updateField("notes", e.target.value)} className="w-full rounded-2xl border-none bg-muted/40 px-6 py-4 text-sm font-bold outline-none ring-2 ring-transparent transition focus:bg-white focus:ring-tropical" placeholder="e.g. Extra sauce" />
                      </div>
                    </div>
                  </div>
                </div>
