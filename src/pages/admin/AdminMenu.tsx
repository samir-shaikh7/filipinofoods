...
import { 
  ...
  Image as ImageIcon, CheckCircle2, Star
} from "lucide-react";
const CATEGORIES = [
...
export function AdminMenu() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Use functional updater to avoid needing formData.category in the dependency array
      if (activeCats.length > 0) {
        setFormData(prev => prev.category ? prev : { ...prev, category: activeCats[0].id });
      }
    } catch (err: any) {
