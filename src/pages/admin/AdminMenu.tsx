...
import { 
  ...
  Image as ImageIcon, CheckCircle2, Star, ChevronDown, Filter, Upload
} from "lucide-react";
...
export function AdminMenu() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filePath = `${activeRestaurant.id}/${fileName}`;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const selectedSection = CATEGORIES.find(c => c.id === formData.category);
      
      const dataToSave = {
        ...formData,
        price: Number(parseFloat(String(formData.price)).toFixed(3)),
        category: selectedSection?.id || CATEGORIES[0].id,
        section_id: selectedSection?.id || CATEGORIES[0].id,
        section_slug: selectedSection?.slug || CATEGORIES[0].slug,
        section_name: selectedSection?.name || CATEGORIES[0].name
      };
    ...
    <AdminLayout title="Menu Management">
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between mb-8">
         ...
         <button
           ...
           className="flex items-center justify-center gap-2 rounded-xl gradient-tropical px-6 py-3 text-sm font-bold text-white shadow-glow"
         >
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="py-20 text-center text-white/20">No items found in this category.</td>
                </tr>
                ...
                <tr key={item.id} className="group hover:bg-white/[0.02]">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2 opacity-40 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleOpenModal(item)} className="p-2 hover:text-mango"><Edit2 className="h-4 w-4" /></button>
                      <button onClick={() => handleDelete(item.id)} className="p-2 hover:text-red-400"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#0A0A0B]/90 backdrop-blur-md animate-fade-in" onClick={() => setIsModalOpen(false)} />
          <div className="relative w-full max-w-xl max-h-[90vh] overflow-y-auto rounded-3xl border border-white/10 bg-[#0D0D0E] shadow-2xl animate-scale-up p-8">
            <div className="flex items-center justify-between mb-8">
              ...
              <button onClick={() => setIsModalOpen(false)} className="p-2 text-white/40"><X /></button>
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-6">
              ...
              <div className="grid gap-6 md:grid-cols-3">
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-white/20">Price ({CURRENCY})</label>
                  <input
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full rounded-xl border border-white/5 bg-white/5 p-4 text-sm focus:border-mango/30 focus:outline-none text-white"
                  >
                    {categories.filter(c => c.id !== "all").map(cat => (
                      <option key={cat.id} value={cat.id} className="bg-white text-charcoal">{cat.name}</option>
                    ))}
                  </select>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col gap-4">
                  {formData.image && (
                    <div className="relative aspect-square w-full max-w-[200px] mx-auto overflow-hidden rounded-[2rem] border border-white/10 shadow-card">
                  ...
                  <div className="flex gap-3">
                    ...
                    <label className="flex shrink-0 cursor-pointer items-center gap-2 rounded-xl border border-white/5 bg-white/5 px-6 py-4 text-sm font-bold transition hover:bg-white/10">
              ...
              <div className="pt-4">
                <button 
                  type="submit" 
                  disabled={saving}
                  className="w-full flex items-center justify-center gap-2 rounded-xl gradient-tropical py-4 text-sm font-bold text-white shadow-glow disabled:opacity-50"
                >
