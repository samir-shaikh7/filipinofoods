...
export function useAppData() {
  ...
  const [reviews, setReviews] = useState<any[]>(() => {
    try {
      const cached = localStorage.getItem(`${CACHE_KEY}_reviews`);
      return cached ? JSON.parse(cached) : [];
    } catch { return []; }
  });
  const [gallery, setGallery] = useState<any[]>(() => {
    try {
      const cached = localStorage.getItem(`${CACHE_KEY}_gallery`);
      return cached ? JSON.parse(cached) : [];
    } catch { return []; }
  });
  ...
  const fetchData = useCallback(async (isSilent = false) => {
    ...
    try {
      ...
      if (reviewsRes.error) {
        console.warn("Reviews fetch failed:", reviewsRes.error.message);
      ...
      if (galleryRes.error) {
        console.warn("Gallery fetch failed:", galleryRes.error.message);
